...
function findSubject(string $subj, array $box): bool
{
    foreach ($box as $value) {
        if (is_array($value)) {
            if (findSubject($subj, $value)) {
                return true;
            }
        } else {
            if ($subj == $value) {
...
$keys = ["Книга", "Фотография", "Нечто", "Поплавок", "Настольная игра", "Ключ"];
foreach ($keys as $key) {
    echo $key . " " . (findSubject($key, $box) ? "есть" : "нет") . PHP_EOL;
}